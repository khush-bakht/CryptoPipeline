#signals/technical_indicator_signal/signal_generator_main
import configparser
import datetime
...
if __name__ == "__main__":
    ...
    config.read("E:\\Neurog\\New\\cryptoPipeline\\signals\\technical_indicator_signal\\signals_config.ini")

    # Read data settings
    data_section = config["DATA"]
    exchange = data_section.get("exchange", "binance")
    symbol = data_section.get("symbol", "btc")
    time_horizon = data_section.get("time_horizon", "5m")
    ...
    df_1m = downloader.download(exchange, symbol, "1m", start_date_str, end_date_str)
    print(df_1m.shape)
    if time_horizon != "1m":
        df = downloader.resample(df_1m, time_horizon)
    else:
        df= df_1m
